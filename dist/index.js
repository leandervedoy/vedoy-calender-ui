...
export function createMeetingForm(onSave) {
    ...
    form.addEventListener("submit", (event) => { event.preventDefault(); const data = new FormData(form); onSave({ id: crypto.randomUUID(), title: String(data.get("title")), startsAt: new Date(String(data.get("startsAt"))).toISOString(), endsAt: new Date(String(data.get("endsAt"))).toISOString(), attendees: String(data.get("attendees")).split(",").map((value) => value.trim()).filter(Boolean), location: String(data.get("location")) || undefined, meetingUrl: String(data.get("meetingUrl")) || undefined, reminderMinutes: Number(data.get("reminderMinutes")), recurrence: data.get("recurrence"), source: "manual" }); form.reset(); });
    return form;
}
